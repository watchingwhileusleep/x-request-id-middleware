...
async def read_root():
    ...
    This function returns the current x-request-ID, which was
    either generated or extracted from the request headers.

    :return: A dictionary containing the x-request-ID.
    """
    return {"x_request_id": get_x_request_id()}
...
def test_fastapi_middleware(client: TestClient) -> None:
    """
    Test that the FastAPI middleware correctly sets the x-request-ID in the
    ...
    assert get_x_request_id() is not None
